...
### Changed
- **Edit mode is refused on a host too old to accept the saves.** A host that sends only the pre-rename save token also sets the owner cookie, so the page used to open fully editable while every save 404'd against a route that host does not have, and an editable page that keeps nothing is worse than a read-only one. Only an explicit `?editmode=true` outranks the check, since that is a person asking on this load rather than a decision baked into a document by an author who could not have known. The on-page notice for this lives in ClayJS; here it stays a console line, because the documents reaching a token-minting host through this library are the tail of a library being retired and a new UI surface would be frozen into every page still loading it.
...
### Fixed
- A save answer is read as text and parsed before the status is consulted. Calling `res.json()` first reported a failure over a save that had actually landed whenever the host answered 200 with an empty body, and turned an intermediary's HTML error page into `Unexpected token '<'` instead of the status that really came back. The status is what is authoritative, and the body may not even be the host's.
