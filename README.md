...
| Module | Size | Description |
|--------|------|-------------|
| ai-edit | 26.5KB | Comment-to-edit AI editing over the local bus: hover chip / ⌘K panel per unit, whole-document bubble, streamed morph previews, one-step undo |
| file-upload | 11.3KB | File upload with progress |
| live-sync | 27.2KB | Real-time DOM sync across browsers (edit mode syncs peers; view mode receives saved updates) |
...
| richclay | 138.1KB | Rich text editing in place: put editable (tokens: single-line, no-toolbar, toolbar-on-select) on any element for a chromeless inline editor with a floating toolbar, or data-richclay for a card editor. Bundles its own Squire + DOMPurify. Edit-mode-only. window.RichClay / window.hyperclay.RichClay. |
